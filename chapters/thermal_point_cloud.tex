...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
		...
		\For {Every image pair $c_i$ captured from the position $p_{c_{i}}$}
	...
	\begin{algorithmic}[1]
		...
		\For {Every image pair $c_i$ captured from the position $p_{c_{i}}$}
		    ...
		    \For {Every 3D candidate point $p_j$}
		        ...
		        \If {Intersected volume is $volume_{p_j}$}
		            \State Calculate 2D \acrshort{tir} point $(x, y)$ (Eq.: \ref{eq:thermal_projection}) %\;
		            \If {$(x, y)$ in \acrshort{tir} polygon}
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=.95\textwidth]{figs/thermal_projection/visualization_thermal_anomalies.png}
...
\marginnote[0cm]{Aside from acceleration, there is little room for improvement in the \acrshort{ecc} image-matching algorithm. However, thermal images were acquired so that the edges from \acrshort{rgb} data were overlapped with infrared data. Although it may help on some datasets, these edges may be too fuzzy for vegetation and hinder the image-matching task. Disabling the use of overlapped data may therefore improve the accuracy of the matching algorithm for this type of datasets. } 
